...
def cleanir(inputfile):
    ...
    if havedq:
        ...
        patternin1[dq1>0] = bad
        patternin2[dq2>0] = bad
        patternin3[dq3>0] = bad
        patternin4[dq4>0] = bad
        print '...flagged', len(patternin1[patternin1==bad]), 'pixels as bad in Q1'
        print '...flagged', len(patternin2[patternin2==bad]), 'pixels as bad in Q2'
        print '...flagged', len(patternin3[patternin3==bad]), 'pixels as bad in Q3'
        print '...flagged', len(patternin4[patternin4==bad]), 'pixels as bad in Q4'

    #-------------------------------------------------------------------
    # Flag pixels above the high limit
    if high:
        print '...flagging high pixels...'
        patternin1[patternin1>high] = bad
        patternin2[patternin2>high] = bad
        patternin3[patternin3>high] = bad
        patternin4[patternin4>high] = bad
        print '...flagged', len(patternin1[patternin1==bad]), 'pixels in Q1'
        print '...flagged', len(patternin2[patternin2==bad]), 'pixels in Q2'
        print '...flagged', len(patternin3[patternin3==bad]), 'pixels in Q3'
        print '...flagged', len(patternin4[patternin4==bad]), 'pixels in Q4'

    #-------------------------------------------------------------------
    # Calculate means and medians for reference (of region to be used for pattern determination):
    ...
    if (biasadjust or graph>0):
        ...
        allpatternin = numpy.concatenate((patternin1,patternin2,patternin3,patternin4))
    ...
    if havedq:
        print '...reflagging bad pixels on cleaned image...'
        dq1 = dq[q1y1:q1y2, q1x1:q1x2]
        dq2 = dq[q2y1:q2y2, q2x1:q2x2]
        dq3 = dq[q3y1:q3y2, q3x1:q3x2]
        dq4 = dq[q4y1:q4y2, q4x1:q4x2]
        ...
        patternin4[dq4>0] = bad
        if verbose:
            print '...reflagged', len(patternin1[patternin1==bad]), 'pixels as bad in Q1'
            print '...reflagged', len(patternin2[patternin2==bad]), 'pixels as bad in Q2'
            print '...reflagged', len(patternin3[patternin3==bad]), 'pixels as bad in Q3'
            print '...reflagged', len(patternin4[patternin4==bad]), 'pixels as bad in Q4'
    ...
    if (biasadjust and not lampsonflat):
        ...
        newpix = numpy.concatenate((
            newimage[q1y1:q1y2, q1x1:q1x2],
            newimage[q2y1:q2y2, q2x1:q2x2],
            newimage[q3y1:q3y2, q3x1:q3x2],
            newimage[q4y1:q4y2, q4x1:q4x2]))
        newmedian, newstddev = IterStat(newpix)
    ...
    if (savepattern):
        ...
        hdu = fits.PrimaryHDU(fullpattern)
